  ...
  if (matches) {
    for (const match of matches) {
      ...
      if (newComponent !== component.full) {
        newChanges = true;
        newFileContents = newFileContents.replace(component.full, newComponent);
      }
    }
    ...
    if (newChanges) {
      try {
        ...
        console.log("\x1b[32m", "    ✔ ", "\x1b[32m", `${file}`, "\x1b[0m");
      } catch (e) {
        console.log(os.EOL);
        console.log("\x1b[31m", "    ✖ ", "\x1b[31m", `${file}`, "\x1b[0m");
        console.log(os.EOL);
        console.log(e);
      }
    } else {
...
injector("./build-tools/include/app-launcher-debug.vbs", (section_name) => {
  switch (section_name) {
    case "PKG":
      return [
        `Shell.ShellExecute scriptDir & "\\required\\${pkg.binaryName}", , , "runas", 1`,
      ];
  }
});
